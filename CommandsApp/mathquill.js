...
//Anat - this is toggle flag 
// If it is true then up or down key was pressed. This allow to prevent clicking up or down without other chars 
var UpDownBlock = false;  

Node.open(function(_) {
  _.keystroke = function(key, e, ctrlr) {
    var cursor = ctrlr.cursor;
    var UpDownHappenned = false; //Anat - this monitor if up or down happened in this click it used to clear the UpDownBlock flag
    ...
    case 'Up': //Anat - changed up cursor to super script
      UpDownHappenned = true;
      if(!UpDownBlock){
    ...
    default:
      if(!UpDownHappenned) // Anat - if the current key is not up or down, clear the flag
        UpDownBlock = false;
      return;
    }
    if(!UpDownHappenned)  // Anat - if the current key is not up or down, clear the flag
...
LatexCmds['\u222b'] =
LatexCmds['int'] =
LatexCmds.integral = bind(Symbol,'\\int ','<span class="mq-int mq-non-leaf "><big>&int;</big></span>');
// LatexCmds['int'] =
